...
class FitGompertzSurvival:
    def __init__(self, rawdata, p0):
        ...
        self.YPredictedValue = self.GompertzSurvivalModelfunc(p[0], p[1], p[2], self.x)
        ...
        for i in range(self.dataLength):
            for j in range(self.pNumber):
                ...
                self.JacobianMatrix[i][j] = (self.GompertzSurvivalModelfunc(self.JDP.jacobianDeltaP[j][0],\
                self.JDP.jacobianDeltaP[j][1], self.JDP.jacobianDeltaP[j][2],self.x[i]) - \
                self.GompertzSurvivalModelfunc(p[0], p[1], p[2], self.x[i]))/self.JDP.dp[j]
    ...
    def GompertzSurvivalModelfunc(self, Y0, mumax, Lag, x):    # x is x data                     
        
        return Y0*(1.0 - np.exp(-np.exp(-(mumax*np.exp(1.0)*(x-Lag)/Y0-1.0))))
        

        
    def YDiff(self, p):
        Y_calculate = self.GompertzSurvivalModelfunc(p[0], p[1], p[2], self.x)
...
def main():
    ...
    rawdata = [x, y]
    p0 = [2.5, 2.5] #p0[0] is rate, p0[1] is lag
    calculation = FitGompertzSurvival(rawdata, p0)
