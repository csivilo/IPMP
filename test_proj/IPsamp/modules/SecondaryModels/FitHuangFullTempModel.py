...
class FitHuangFullTemp:
    def __init__(self, rawdata, p0):
        ...
        self.YPredictedValue = self.HuangFullTempModelfunc(p[0], p[1], p[2], p[3], self.x)
        self.Xplot = np.linspace(p[0], p[1], 101)
        self.Yplot = self.HuangFullTempModelfunc(p[0], p[1], p[2], p[3], self.Xplot)
        ...
        for i in range(self.dataLength):
            for j in range(self.pNumber):
                ...
                self.HuangFullTempModelfunc(self.JDP.jacobianDeltaP[j][0],\
                self.JDP.jacobianDeltaP[j][1], self.JDP.jacobianDeltaP[j][2],\
                self.JDP.jacobianDeltaP[j][3],self.x[i]) - \
                self.HuangFullTempModelfunc(p[0], p[1], p[2], p[3], self.x[i]))/self.JDP.dp[j]
    ...
    def HuangFullTempModelfunc(self, T0, Tmax, a, b, x):    # x is x data
        
        return  a*(x - T0)**0.75*(1.0 - np.exp(b*(x - Tmax)))
        

        
    def YDiff(self, p):
        Y_calculate = self.HuangFullTempModelfunc(p[0], p[1], p[2], p[3], self.x)
...
def main():
    ...
    rawdata = [x, y]
    p0 = [5.5, 40., 0.20, 0.10] #p0[0] is rate, p0[1] is lag
    calculation = FitHuangFullTemp(rawdata, p0)
