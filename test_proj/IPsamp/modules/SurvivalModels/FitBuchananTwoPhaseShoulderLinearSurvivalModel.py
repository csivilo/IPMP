...
class FitBuchananTwoPhaseShoulderLinearSurvival:
    def __init__(self, rawdata, p0):
        ...
        self.YPredictedValue = self.BuchananTwoPhaseShoulderLinearSurvivalModelfunc(p[0], p[1], p[2], self.x)
        ...
        for i in range(self.dataLength):
            for j in range(self.pNumber):
                ...
                self.BuchananTwoPhaseShoulderLinearSurvivalModelfunc(self.JDP.jacobianDeltaP[j][0],\
                self.JDP.jacobianDeltaP[j][1], self.JDP.jacobianDeltaP[j][2],self.x[i]) - \
                self.BuchananTwoPhaseShoulderLinearSurvivalModelfunc(p[0], p[1], p[2], self.x[i]))/self.JDP.dp[j]
    ...
    def BuchananTwoPhaseShoulderLinearSurvivalModelfunc(self, y0, k, lag, x):
        
        a1 = np.piecewise(x, [x < lag, x >= lag], [0., 1.])
        
        return y0*(1.-a1)+(y0 - (x-lag)/k)*a1
        

        
    def YDiff(self, p):
        Y_calculate = self.BuchananTwoPhaseShoulderLinearSurvivalModelfunc(p[0], p[1], p[2], self.x)
...
def main():
    ...
    calculation = FitBuchananTwoPhaseShoulderLinearSurvival(rawdata, p0)
