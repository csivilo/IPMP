...
class FitWeibullSurvival:
    def __init__(self, rawdata, p0):
        ...
        self.YPredictedValue = self.WeibullSurvivalModelfunc(p[0], p[1], p[2], self.x)
        ...
        for i in range(self.dataLength):
            for j in range(self.pNumber):
                ...
                self.WeibullSurvivalModelfunc(self.JDP.jacobianDeltaP[j][0],\
                self.JDP.jacobianDeltaP[j][1], self.JDP.jacobianDeltaP[j][2], self.x[i]) - \
                self.WeibullSurvivalModelfunc(p[0], p[1], p[2], self.x[i]))/self.JDP.dp[j]
    ...
    def WeibullSurvivalModelfunc(self, Y0, K, alpha, x):    # x is x data
    ...
    def YDiff(self, p):
        Y_calculate = self.WeibullSurvivalModelfunc(p[0], p[1], p[2], self.x)
...
def main():
    ...
    calculation = FitWeibullSurvival(rawdata, p0)
