...
class FitHuangSubTemp:
    def __init__(self, rawdata, p0):
        ...
        self.YPredictedValue = self.HuangSubTempModelfunc(p[0], p[1], self.x)
        self.Xplot = np.linspace(p[0], np.max(self.x)*1.05, 501)
        self.Yplot = self.HuangSubTempModelfunc(p[0], p[1], self.Xplot)
        ...
        for i in range(self.dataLength):
            for j in range(self.pNumber):
                ...
                self.HuangSubTempModelfunc(self.JDP.jacobianDeltaP[j][0],\
                self.JDP.jacobianDeltaP[j][1], self.x[i]) - \
                self.HuangSubTempModelfunc(p[0], p[1], self.x[i]))/self.JDP.dp[j]
    ...
    def HuangSubTempModelfunc(self, T0, a, x):    # x is x data
    ...
    def YDiff(self, p):
        Y_calculate = self.HuangSubTempModelfunc(p[0], p[1], self.x)
...
def main():
    ...
    calculation = FitHuangSubTemp(rawdata, p0)
