...
class FitBaranyi:
    def __init__(self, rawdata, p0):
        ...
        self.YPredictedValue = self.BaranyiModelfunc(p[0], p[1], p[2], p[3], self.x)
        ...
        for i in range(self.dataLength):
            for j in range(self.pNumber):
                ...
                self.BaranyiModelfunc(p[0], p[1], p[2], p[3], self.x[i]))/self.JDP.dp[j]
...
def main():
    ...
    rawdata = [x, y]
    p0 = [2.5, 3.5] #p0[0] is rate, p0[1] is lag
    calculation = FitBaranyi(rawdata, p0)
